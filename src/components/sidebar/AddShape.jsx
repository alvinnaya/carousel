...
const AddShape = () => {
    ...
    return (
        <div className="space-y-6">
            <section>
                <h3 className="text-[10px] font-black mus-text-muted uppercase tracking-widest mb-3">Basic Shapes</h3>
                <div className="grid grid-cols-3 gap-3">
                    {['rect', 'circle', 'triangle', 'line', 'star', 'hexagon'].map((shape) => (
                        <div
                            key={shape}
                            onClick={() => handleAddShape(shape)}
                            className="aspect-square mus-card flex items-center justify-center cursor-pointer p-3"
                        >
                            <div className={`w-full h-full border-2 border-[#1A1A1A] ${shape === 'circle' ? 'rounded-full' : ''}`} />
                        </div>
                    ))}
                </div>
            </section>
