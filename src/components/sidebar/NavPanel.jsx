...
const NavPanel = ({ activeTab, onTabClick }) => {
    ...
    return (
        <aside className="h-full w-16 flex flex-col mus-panel overflow-hidden pointer-events-auto">
            <Logo />

            <nav className="flex-1 flex flex-col items-center py-2 space-y-2 overflow-y-auto no-scrollbar">
                {tools.map((tool) => (
                    <SidebarItem
                        key={tool.name}
                        icon={tool.icon}
